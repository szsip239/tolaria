...
type CollectionPresentationConfig =
  ...
  | { type: 'custom'; provider: string; options: Record<string, unknown> }
```

`src/collections/resolveCollectionEntries.ts` resolves the selected collection through the existing note-list filtering rules. A saved collection carries its parsed `ViewFile`, so alternate presentations consume the same filtered entries without receiving or re-parsing the global view registry. Changes and Inbox stay caller-supplied transient flows because they depend on git state and inbox-period state outside saved-view YAML. Neighborhood resolves to grouped relationship data instead of a flat row list.

`CollectionPresentationHost` is an internal, compile-time provider boundary for non-list surfaces. A registered provider receives the collection, resolved vault entries, loading/vault context, and narrow note read/write/refresh/navigation callbacks. Navigation resolves only indexed notes, returns to the normal entity/editor workspace, and can request a stable text anchor without exposing tab or editor internals to the provider. `editorAnchorNavigation` keeps that request pending across the collection-to-editor remount; `SingleEditorView` consumes it only when the target note and matching BlockNote block are present, then centers the block. Saved YAML selects a provider by stable id but cannot load code. Missing or unknown providers fall back to the ordinary list/editor path. This lets future presentations such as board, calendar, table, timeline, graph, or review surfaces consume the same resolved collection model without branching directly on sidebar state or acquiring unrestricted filesystem access.

### Saved Views

Saved Views live as YAML files under `views/`. Their definition includes user-visible fields (`name`, `icon`, `color`), note-list preferences (`sort`, `listPropertiesDisplay`), filters, an optional top-level `order` number, and optional nested `presentation` configuration. The renderer treats saved Views as the most configurable persisted Collection artifact. Existing top-level `sort` and `listPropertiesDisplay` fields normalize into the list presentation config; nested `presentation.type: list` may override them, while `presentation.type: custom` preserves a registered provider id and provider-owned options. Rust view parsing keeps unknown provider option fields intact when loading and saving YAML. The `sort` value accepts built-in sort forms such as `"modified:desc"` and custom-property forms such as `"property:Priority:asc"` or bare `"Priority:asc"`; the renderer keeps configured custom-property sorts visible even when the current result set has no populated values for that property. Filter conditions on scalar-array custom properties, such as `tags: [blues, chicago]`, evaluate `contains`, `any_of`, and related set operators against exact array elements rather than substrings. The `order` value is stored directly in the YAML document, not in Markdown frontmatter, and lower values render earlier in every saved-View list. Views without an explicit order sort after ordered views by filename for stable fallback behavior.
